...
constexpr int D(10); //delay (how many time after the post-synpatic neuron receives a spike) (=1.0 in milliseconds)
...
public:
	...
	void update(int dt);	
	void receive (int time, double J); //if the neuron is a post-synaptic neuron, and it receives an input from another neuron (at time time and with amplitude J), this function update its state
